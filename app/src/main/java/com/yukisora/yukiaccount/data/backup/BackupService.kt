...
class BackupService(
    ...
    private fun BackupDocument.duplicateIdReason(): String? {
        listOf(
            ...
            ?.let { (label, _) -> return "备份文件包含重复${label} ID" }

        val automaticOccurrences = transactions
            .filter { it.autoGenerated }
            .map { requireNotNull(it.recurringRuleId) to requireNotNull(it.occurrenceDate) }
        if (automaticOccurrences.hasDuplicate()) {
            return "备份文件包含重复自动周期流水"
        }

        val skippedOccurrenceKeys = skippedOccurrences.map { it.recurringRuleId to it.occurrenceDate }
        if (skippedOccurrenceKeys.hasDuplicate()) {
            return "备份文件包含重复跳过周期记录"
        }
    ...
    private fun BackupDocument.semanticIntegrityReason(): String? {
        ...
        transactions.forEach { transaction ->
            ...
            transaction.categoryId?.let { categoryId ->
                if (categoryTypes.getValue(categoryId) != transactionType.categoryType()) {
                    return "备份文件包含分类类型不匹配的流水"
                }
            }
        ...
        recurringRules.forEach { rule ->
            ...
            val transactionType = TransactionType.valueOf(rule.transactionType)
            when (transactionType) {
    ...
    private companion object {
        val CATEGORY_TYPES = setOf("expense", "income", "transfer", "investment")
    }
}
